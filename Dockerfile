FROM ubuntu:xenial
ENV TRAC_ADMIN_NAME dani
ENV TRAC_ADMIN_PASSWD password1
ENV TRAC_PROJECT_NAME EPLL
ENV TRAC_DIR /var/local/trac
ENV TRAC_INI $TRAC_DIR/conf/trac.ini
ENV DB_LINK sqlite:db/trac.db
EXPOSE 8123

RUN apt-get update && apt-get install -y trac python-babel libapache2-mod-wsgi python-pip && apt-get -y clean
